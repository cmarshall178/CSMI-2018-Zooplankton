...
<<<<<<< HEAD
---
title: "Zooplankton 2018 Ontario Analysis"
...
Zoop_Bio_Den_New <- read_csv("C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Updated Raw Data/Zoop_Bio_Den_New.csv")

Areal_Bio_Den_Zoop <- read_csv("C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Updated Raw Data/Areal_Bio_Den_Zoop.csv")

#Invasive_Zoops_JKC<- read_csv("C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Updated Raw Data/Station_Data_Summer_JKC.csv")
...
Basin <- Zoop_Bio_Den_New$Basin

Whole_CountType <- Areal_Bio_Den_Zoop$CountType
Whole_Strata <- Areal_Bio_Den_Zoop$Strata
Whole_SPECCODE <- Areal_Bio_Den_Zoop$SPECCODE
Whole_SUMGROUP <- Areal_Bio_Den_Zoop$SUMGROUP
Whole_Density <- Areal_Bio_Den_Zoop$`Areal Density m2`
Whole_Biomass <- Areal_Bio_Den_Zoop$`Areal Biomass m2`
Whole_Station <- Areal_Bio_Den_Zoop$Station
Whole_DN <- Areal_Bio_Den_Zoop$DN
Whole_EventID <- Areal_Bio_Den_Zoop$EventCode
Whole_Basin <- Areal_Bio_Den_Zoop$Basin
```
...
Areal_Epi_Filter <- filter(Areal_Bio_Den_Zoop, Strata == "epi")

# Filter for Macro CountType
Epi_Macro_Filter <- filter(Epi_Filter, CountType == "MACRO")

Areal_Epi_Macro_Filter <- filter(Areal_Epi_Filter, CountType == "MACRO")

# Removal of Macro Nauplii from Macro filter
Epi_Macro_Filter <- Epi_Macro_Filter[-c(487, 514, 526, 530, 576, 585, 593, 608),]

# Filter for Micro CountType  
Epi_Micro_Filter <- filter(Epi_Filter, CountType == "MICRO")

Areal_Epi_Micro_Filter <- filter(Areal_Epi_Filter, CountType == "MICRO")

# Completion of Zoop_Bio_Den dataset with implicit zeros for each Zoop_GENERA for each month split by Macro and Micro
############Checking Epi Sumgroup data instead of Zoop_GENERA
Epi_Macro_Zeros <- Epi_Macro_Filter %>% 
    filter(SPECCODE != "LETKIND" & SPECCODE != "POYPEDI" & SPECCODE != "VELIGER" & SPECCODE != "ROTIFER") %>% 
    group_by(SUMGROUP, Season, Station, EventID, DN, Lat, Long, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long, Basin), SUMGROUP, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))

#removing most unnecessary taxa for now 2/26/2023 - consider separating Byth/Cerc as well

Areal_Epi_Macro_Zeros <- Areal_Epi_Macro_Filter %>% 
    filter(SPECCODE != "LETKIND" & SPECCODE != "POYPEDI" & SPECCODE != "MYSRELI" & SPECCODE != "HARCOPE" & SPECCODE != "DIPSP") %>% 
    group_by(SUMGROUP, Season, Station, EventCode, DN, Lat, Long, Transect, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(`Areal Biomass m2`), Zoop_Den_Sum = sum(`Areal Density m2`)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventCode, DN, Lat, Long, Transect, Basin), SUMGROUP, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))


Epi_Micro_Zeros <- Epi_Micro_Filter %>% 
    group_by(SUMGROUP, Season, Station, EventID, DN, Lat, Long, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long, Basin), SUMGROUP, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))

Areal_Epi_Micro_Zeros <- Areal_Epi_Micro_Filter %>% 
    group_by(SUMGROUP, Season, Station, EventCode, DN, Lat, Long, Transect, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(`Areal Biomass m2`), Zoop_Den_Sum = sum(`Areal Density m2`)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventCode, DN, Lat, Long, Transect, Basin), SUMGROUP, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))
...
Complete_Epi_Zoop_Bio_Den_New <- full_join(Epi_Macro_Zeros, Epi_Micro_Zeros, copy = F)

Complete_Areal_Epi_Bio_Den_Zoop <- full_join(Areal_Epi_Macro_Zeros, Areal_Epi_Micro_Zeros, copy = F)

#write.csv(Complete_Epi_Zoop_Bio_Den_New, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Complete_Epi_Zoop_Bio_Den_New.csv")

#write.csv(Complete_Areal_Epi_Bio_Den_Zoop, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Complete_Areal_Epi_Bio_Den_Zoop.csv")
```


```{r echo = TRUE, warning = FALSE}
#Checking Epi SPECCODE data 
Whole_Epi_Macro_Zeros <- Epi_Macro_Filter %>% 
    group_by(SPECCODE, Season, Station, EventID, DN, Lat, Long, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long, Basin), SPECCODE, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))

Areal_Whole_Epi_Macro_Zeros <- Areal_Epi_Macro_Filter %>% 
    group_by(SPECCODE, Season, Station, EventCode, DN, Lat, Long, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(`Areal Biomass m2`), Zoop_Den_Sum = sum(`Areal Density m2`)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventCode, DN, Lat, Long, Basin), SPECCODE, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))

################################################################################################
#filter(SPECCODE != "LETKIND" & SPECCODE != "POYPEDI" & SPECCODE != "VELIGER" & SPECCODE != "ROTIFER")

Whole_Epi_Micro_Zeros <- Epi_Micro_Filter %>% 
    group_by(SPECCODE, Season, Station, EventID, DN, Lat, Long) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long), SPECCODE, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))

# Joining Micro and Macro values after including implicit zeros for each Zoop_GENERA per EventCodeKey(proxy for station)
Whole_Complete_Epi_Zoop_Bio_Den_New <- full_join(Whole_Epi_Macro_Zeros, Whole_Epi_Micro_Zeros, copy = F)
...
```{r Completing the dataframes with implicit zeros, echo = TRUE, warning = FALSE}
# Filters for WWC Strata 
WWC_Filter <- filter(Zoop_Bio_Den_New, Strata == "wwc")

# Completion of Zoop_Bio_Den dataset with implicit zeros for each Zoop_GENERA for each month split by Macro and Micro
Complete_WWC_Zoop_Bio_Den_New <- WWC_Filter %>% 
    group_by(SUMGROUP, Season, Station, EventID, DN, Lat, Long, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long, Basin), SUMGROUP, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))

#write.csv(Complete_WWC_Zoop_Bio_Den_New, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Complete_WWC_Zoop_Bio_Den_New.csv")

Areal_WWC_Filter <- filter(Areal_Bio_Den_Zoop, Strata == "wwc")

# Completion of Zoop_Bio_Den dataset with implicit zeros for each Zoop_GENERA for each month split by Macro and Micro
Areal_WWC_Zoop_Bio_Den <- Areal_WWC_Filter %>% 
   filter(SPECCODE != "LETKIND" & SPECCODE != "POYPEDI" & SPECCODE != "MYSRELI" & SPECCODE != "HARCOPE" & SPECCODE != "DIPSP") %>%
    group_by(SUMGROUP, Season, Station, EventCode, DN, Lat, Long, Transect, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(`Areal Biomass m2`), Zoop_Den_Sum = sum(`Areal Density m2`)) %>% 
      ungroup %>% 
...
#write.csv(Areal_WWC_Zoop_Bio_Den, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Areal_WWC_Zoop_Bio_Den.csv")
```
...
# Completion of Zoop_Bio_Den dataset with implicit zeros for each Zoop_GENERA for each month split by Macro and Micro
Whole_Complete_WWC_Zoop_Bio_Den_New <- WWC_Filter %>% 
    group_by(SPECCODE, Season, Station, EventID, DN, Lat, Long, Basin) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long, Basin), SPECCODE, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))
...
##############Code edited for SUMGROUP instead of Zoop_GENERA########### Also excluded micro sumgroup data ### Excluding Pred Cla 2/28/2023
Epi_Sum_Stats <- Complete_Epi_Zoop_Bio_Den_New %>% 
  filter(SUMGROUP != "OTHER" & SUMGROUP != "MYSID" & SUMGROUP != "VELIGER" & SUMGROUP != "ROTIFER" & SUMGROUP != "pNAUPLII" & SUMGROUP != "BYTH" & SUMGROUP != "CERC") %>% 
  group_by(SUMGROUP, Season, Basin) %>%
...
#Invasive_Zoops_JKC_Sum_Stats <- Invasive_Zoops_JKC %>% 
  #group_by(Species, Year) %>%
  #summarise(All_Bio_SUM = sum(`Biomass ug_m3`), 
            #All_Den_SUM = sum(`Density_m3`),
            #All_Bio_MEAN = mean(`Biomass ug_m3`), 
            #All_Den_MEAN = mean(`Density_m3`), 
            #All_Bio_SD = sd(`Biomass ug_m3`), 
            #All_Den_SD = sd(`Density_m3`),
            #N = n(),
            #All_Bio_SE = All_Bio_SD / sqrt(N), 
            #All_Den_SE = All_Den_SD / sqrt(N))

#filter(MONTH != "Oct")
#filter for only Byth and Cerc
Whole_Epi_Sum_Stats <- Whole_Complete_Epi_Zoop_Bio_Den_New %>% 
  filter(SPECCODE == "BYTLONG" | SPECCODE == "CECPENG" | SPECCODE == "LETKIND" | SPECCODE == "POYPEDI") %>% 
  group_by(SPECCODE, Season, Basin) %>%
  summarise(All_Bio_SUM = sum(Zoop_Bio_Sum), 
            All_Den_SUM = sum(Zoop_Den_Sum),
            All_Bio_MEAN = mean(Zoop_Bio_Sum), 
            All_Den_MEAN = mean(Zoop_Den_Sum), 
...
Areal_Epi_Sum_Stats <- Complete_Areal_Epi_Bio_Den_Zoop %>% 
  filter(SUMGROUP != "VELIGER" & SUMGROUP != "ROTIFER" & SUMGROUP != "pNAUPLII" & SUMGROUP != "BYTH" & SUMGROUP != "CERC") %>% 
  group_by(Season, SUMGROUP, Basin) %>%
  summarise(All_Bio_SUM = sum(Zoop_Bio_Sum), 
            All_Den_SUM = sum(Zoop_Den_Sum),
            All_Bio_MEAN = mean(Zoop_Bio_Sum), 
            All_Den_MEAN = mean(Zoop_Den_Sum), 
...
#write.csv(Epi_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Epi_Sum_Stats.csv")

#write.csv(Whole_Epi_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/PRED_CLA_Epi_Sum_Stats.csv")

#write.csv(Areal_Epi_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Areal_Epi_Sum_Stats.CSV")

#Not finished with this code yet
#Stats for all epi zooplankton figures with SE bars
Epi_All_SUM <- Complete_Epi_Zoop_Bio_Den_New %>%
  filter(SUMGROUP != "OTHER" & SUMGROUP != "MYSID" & SUMGROUP != "VELIGER" & SUMGROUP != "ROTIFER" & SUMGROUP != "pNAUPLII") %>% 
  group_by(EventID, Season, Basin) %>% 
...
#Invasive_Zoops_JKC_All_SUM <- Invasive_Zoops_JKC %>%
  #group_by(Station, Year) %>% 
  #summarise(Zoop_Bio_All = sum(`Biomass ug_m3`),
            #Zoop_Den_All = sum(`Density_m3`)) %>% 
            #ungroup %>% 
            #complete(nesting(Year, Station), fill = list(Zoop_Bio_All = 0, Zoop_Den_All = 0))
              
#filter(DN == "D" | DN == "N") %>% filter(MONTH != "Oct") %>% 
####################Late fall samples only include micro - haven't included areal density caculations for macro samples yet.

Epi_All_Stats <- Epi_All_SUM %>%
...
#Invasive_Zoops_JKC_All_Stats <- Invasive_Zoops_JKC_All_SUM %>%
  #group_by(Year) %>%
  #summarise(Zoop_All_Bio_SUM = sum(Zoop_Bio_All),
            #Zoop_All_Abun_SUM = sum(Zoop_Den_All),
            #Zoop_All_Bio_MEAN = mean(Zoop_Bio_All), 
...
#write.csv(Epi_All_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/EPI/Epi_All_Stats.csv")
```


```{r Calculating summary stats (sum/mean/sd/se) for each Zoop_GENERA per month, echo = TRUE, warning = FALSE}
# Applying summary sum/mean/sd values for each Zoop_GENERA per month; note the proper N values for each month for both Macro and Micro Taxa
WWC_Sum_Stats <- Complete_WWC_Zoop_Bio_Den_New %>% 
  filter(SUMGROUP != "OTHER" & SUMGROUP != "MYSID" & SUMGROUP != "BYTH" & SUMGROUP != "CERC") %>% 
  group_by(SUMGROUP, Season, Basin) %>%
  summarise(All_Bio_SUM = sum(Zoop_Bio_Sum), 
            All_Den_SUM = sum(Zoop_Den_Sum),
            All_Bio_MEAN = mean(Zoop_Bio_Sum), 
...
# Ordering of SUMGROUP column

Whole_WWC_Sum_Stats <- Whole_Complete_WWC_Zoop_Bio_Den_New %>% 
  filter(SPECCODE == "BYTLONG" | SPECCODE == "CECPENG" | SPECCODE == "LETKIND" | SPECCODE == "POYPEDI") %>% 
  group_by(SPECCODE, Season, Basin) %>%
  summarise(All_Bio_SUM = sum(Zoop_Bio_Sum), 
            All_Den_SUM = sum(Zoop_Den_Sum),
...
Areal_WWC_Sum_Stats <- Areal_WWC_Zoop_Bio_Den %>% 
  filter(SUMGROUP != "BYTH" & SUMGROUP != "CERC" & SUMGROUP != "OTHER") %>% 
  group_by(SUMGROUP, Season, Basin) %>%
  summarise(All_Bio_SUM = sum(Zoop_Bio_Sum), 
            All_Den_SUM = sum(Zoop_Den_Sum),
            All_Bio_MEAN = mean(Zoop_Bio_Sum), 
...
WWC_Sum_Stats$SUMGROUP <- factor(WWC_Sum_Stats$SUMGROUP, levels = c("LIMNO", "CALANOID", "CYCLOPOID", "DAPHNID", "BOSMINID", "NDAP"))

Whole_WWC_Sum_Stats$SPECCODE <- factor(Whole_WWC_Sum_Stats$SPECCODE, levels = c("BYTLONG", "CECPENG", "LETKIND", "POYPEDI"))

Areal_WWC_Sum_Stats$SUMGROUP <- factor(Areal_WWC_Sum_Stats$SUMGROUP, levels = c("LIMNO", "CALANOID", "CYCLOPOID", "DAPHNID", "BOSMINID", "NDAP"))

#write.csv(WWC_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Basin_WWC_Sum_Stats")

#write.csv(Whole_WWC_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Basin_WWC_Sum_Stats")

#write.csv(Areal_WWC_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Basin_Areal_WWC_Sum_Stats")

###WWC_Sum_Stats$SUMGROUP <- factor(WWC_Sum_Stats$SUMGROUP, levels = c("LETKIND", "HOLGIBB", "POYPEDI", "BOSLONG", "EUBCORE", "DAPSP", "DAPRETR", "DAPGALEM", "BYTLONG", "CECPENG", "DIYTHOM", "CYCCOPE", "MESCOPE", "MESEDAX" "TROPRASM", "DIACOPE", "LEOASHL", "LEOMINU", "LEOSICI", "SKIOREG", "EPILACU", "EPICOPE", "LIMCOPE", "LIMMACR"))

#Not finished with this code yet
#Stats for all epi zooplankton figures with SE bars
WWC_All_SUM <- Complete_WWC_Zoop_Bio_Den_New %>%
  filter(SUMGROUP != "OTHER" & SUMGROUP != "MYSID" & SUMGROUP != "VELIGER" & SUMGROUP != "ROTIFER" & SUMGROUP != "pNAUPLII" & SUMGROUP != "BYTH" & SUMGROUP != "CERC") %>% 
  group_by(EventID, Season, Basin) %>% 
  summarise(Zoop_Bio_All = sum(Zoop_Bio_Sum),
            Zoop_Den_All = sum(Zoop_Den_Sum)) %>% 
...
#write.csv(WWC_All_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/WWC_Sum_Stats")
```


```{r Plotting Biomass and Density Averages for All WWC Taxa, echo = FALSE, warning = FALSE, fig.width = 7, fig.height = 5}

Whole_WWC_Sum_Stats$Season <- factor(Whole_WWC_Sum_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall", "Late Fall"))

WWC_Sum_Stats$Season <- factor(WWC_Sum_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall", "Late Fall"))

Areal_WWC_Sum_Stats$Season <- factor(Areal_WWC_Sum_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall", "Late Fall"))

#Invasive_Zoops_JKC_Sum_Stats$Year <- factor(Invasive_Zoops_JKC_Sum_Stats$Year, levels = c(2012,2013,2014,2015,2016,2017,2018,2019))

# Structure for Barbiero similar color palette
Barb_Palette <- c("blue2", "cyan3", "yellow2", "red2", "seagreen4", "mediumorchid4")
Pred_Cla_Palette <- c("orange3", "gray0", "green2", "navy")
...
gg_BIO_All <- ggplot(Areal_WWC_Sum_Stats, aes(x = Season, y = All_Bio_MEAN, fill = SUMGROUP)) + 
  geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
  facet_wrap(~Basin) +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 26), plot.title = element_blank()) +
#legend details 
  theme(legend.title = element_blank(), legend.position = "none", legend.box.background = element_rect(), legend.box.margin = margin(3,3,3,3), legend.text = element_text(size = 20)) +
#palette color for bar fills; see Barbiero for reference
  scale_fill_manual(values = c(Barb_Palette)) +
  labs(x = "Month", y = "Biomass (m2)") 
  #ylim(0,1.5)

#element_text(size = 22, angle = 45, vjust = 1, hjust = 1)
#plot the above graph
gg_BIO_All

#ggsave(filename = 'Basin_Areal_WWC_Bio_Sum_Stats.TIFF', plot = gg_BIO_All, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/WWC/Areal")

#plotting average density for all taxa from epi per month
gg_DEN_All <- ggplot(Areal_WWC_Sum_Stats, aes(x = Season, y = All_Den_MEAN, fill = SUMGROUP))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
  facet_wrap(~Basin) +
...
#palette color for bar fills; see Barbiero for reference
  scale_fill_manual(values = c(Barb_Palette)) +
  labs(x = "Month", y = "Abundance (m2)") 
  
  #scale_y_sqrt() +
  #scale_y_break(c(2000, 5000))
  #ylim(0,0.3)

#plot the above graph
gg_DEN_All 

#ggsave(filename = 'Basin_Areal_WWC_Den.TIFF', plot = gg_DEN_All, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/WWC/Areal")
```


```{r zoop genera relative abundance transformation code, echo = FALSE, warning = FALSE}
#code used to group the Zoop_Sum_Stats Mean abundance data for each zoop genera per month and mutate (transform) the mean values into relative abundances
WWC_Zoop_Prop <-
  Whole_WWC_Sum_Stats %>%
    select(SPECCODE, Season, Basin, All_Bio_MEAN, All_Den_MEAN) %>%
    group_by(Season, Basin)%>%
    mutate(Zoop_Prop_Bio = (All_Bio_MEAN / sum(All_Bio_MEAN) * 100), Zoop_Prop_Abun = (All_Den_MEAN / sum(All_Den_MEAN) * 100))

Areal_WWC_Zoop_Prop <-
  Areal_WWC_Sum_Stats %>%
    select(SUMGROUP, Season, Basin, All_Bio_MEAN, All_Den_MEAN) %>%
    group_by(Season, Basin)%>%
    mutate(Zoop_Prop_Bio = (All_Bio_MEAN / sum(All_Bio_MEAN) * 100), Zoop_Prop_Abun = (All_Den_MEAN / sum(All_Den_MEAN) * 100))

#write.csv(WWC_Zoop_Prop, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Basin_Pred_Cla_WWC_Prop")

#write.csv(Areal_WWC_Zoop_Prop, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Basin_Areal_WWC_Prop")
```


```{r zoop genera relative abundance and biomass stack graph, echo = FALSE, warning = FALSE, fig.width = 7, fig.height = 5}

Areal_WWC_Zoop_Prop$Season <- factor(WWC_Zoop_Prop$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall", "Late Fall"))

gg_Rel_Zoop_Bio <- ggplot(WWC_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Bio, fill = SPECCODE)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  facet_wrap(~Basin) +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 26), plot.title = element_blank()) +
#legend details 
  theme(legend.title = element_blank(), legend.position = "none", legend.box.background = element_rect(), legend.box.margin = margin(3,3,3,3), legend.text = element_text(size = 14)) +
#palette color for bar fills; see Barbiero for reference
  scale_fill_manual(values = c(Pred_Cla_Palette)) +
  labs(x = "Month", y = "% Biomass") 

gg_Rel_Zoop_Bio

ggsave(filename = 'Basin_WWC_Bio_Zoop_Prop.TIFF', plot = gg_Rel_Zoop_Bio, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/WWC/Cubic")

gg_Rel_Zoop_Den <- ggplot(WWC_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Abun, fill = SPECCODE)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  facet_wrap(~Basin) +
  #panel border, axis text and title sizes and orientations
...
#ggsave(filename = 'Basin_Areal_WWC_Den_Zoop_Prop.TIFF', plot = gg_Rel_Zoop_Den, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/WWC/Areal")

WWC_All_Zoop_Figure <- ggarrange(gg_DEN_All, gg_BIO_All, gg_Rel_Zoop_Den, gg_Rel_Zoop_Bio, 
        labels = c("A", "B", "C", "D"),
...
#ggsave(filename = 'Basin_Areal_WWC_Zoop_Grid.TIFF', plot = WWC_All_Zoop_Figure, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/WWC/Cubic")
```
...
######## Limnocalanus not removed from samples yet###################################

Whole_Epi_Sum_Stats$SPECCODE <- factor(Whole_Epi_Sum_Stats$SPECCODE, levels = c("BYTLONG", "CECPENG", "LETKIND", "POYPEDI"))

Whole_Epi_Sum_Stats$Season <- factor(Whole_Epi_Sum_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall"))

Epi_Sum_Stats$SUMGROUP <- factor(Epi_Sum_Stats$SUMGROUP, levels = c("LIMNO", "CALANOID", "CYCLOPOID", "CERC", "BYTH",  "DAPHNID", "BOSMINID", "NDAP"))

Epi_Sum_Stats$Season <- factor(Epi_Sum_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall"))

Areal_Epi_Sum_Stats$SUMGROUP <- factor(Areal_Epi_Sum_Stats$SUMGROUP, levels = c("LIMNO", "CALANOID", "CYCLOPOID", "DAPHNID", "BOSMINID", "NDAP"))

Areal_Epi_Sum_Stats$Season <- factor(Areal_Epi_Sum_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall"))

# Structure for Barbiero similar color palette
#Barb_Palette <- c("blue2", "cyan3", "yellow2", "orange3", "saddlebrown", "red2", "seagreen4", "mediumorchid4")
Barb_Palette <- c("blue2", "cyan3", "yellow2", "red2", "seagreen4", "mediumorchid4")
...
#plotting average biomass for all taxa from epi per month 
gg_Epi_BIO_All <- ggplot(Areal_Epi_Sum_Stats, aes(x = Season, y = All_Bio_MEAN, fill = SUMGROUP)) + 
  geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
#legend details 
  theme(legend.title = element_blank(), legend.position = "none", legend.box.background = element_rect(), legend.box.margin = margin(3,3,3,3), legend.text = element_text(size = 20)) +
#palette color for bar fills; see Barbiero for reference
  scale_fill_manual(values = c(Barb_Palette)) +
...
#X axis text size and angle
##size = 24, angle = 45, vjust = 1, hjust = 1
#plot the above graph
gg_Epi_BIO_All

#ggsave(filename = 'Areal_Epi_Bio_Stats.TIFF', plot = gg_Epi_BIO_All, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/EPI/Areal")


#plotting average density for all taxa from epi per month
gg_Epi_DEN_All <- ggplot(data = Areal_Epi_Sum_Stats, aes(x = Season, y = All_Den_MEAN, fill = SUMGROUP))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
...
#ggsave(filename = 'Areal_Epi_Den_Stats.TIFF', plot = gg_Epi_DEN_All, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/EPI/Areal")
```
...
Epi_Zoop_Prop <-
  Epi_Sum_Stats %>%
    select(SUMGROUP, Season, All_Bio_MEAN, All_Den_MEAN) %>%
    group_by(Season)%>%
    mutate(Zoop_Prop_Bio = (All_Bio_MEAN / sum(All_Bio_MEAN) * 100), Zoop_Prop_Abun = (All_Den_MEAN / sum(All_Den_MEAN) * 100))

#write.csv(WWC_Zoop_Prop, file = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Processed Data/WWC_Zoop_Prop")
...
#write.csv(Areal_Epi_Zoop_Prop, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Areal/Areal_Epi_Zoop_Prop")
```
...
Areal_Epi_Zoop_Prop$SUMGROUP <- factor(Areal_Epi_Zoop_Prop$SUMGROUP, levels = c("LIMNO", "CALANOID", "CYCLOPOID", "DAPHNID", "BOSMINID", "NDAP"))

Areal_Epi_Zoop_Prop$Season <- factor(Areal_Epi_Zoop_Prop$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall"))


gg_Rel_Epi_Zoop_Bio <- ggplot(Areal_Epi_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Bio, fill = SUMGROUP)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
#legend details 
  theme(legend.title = element_blank(), legend.position = "none", legend.box.background = element_rect(), legend.box.margin = margin(3,3,3,3), legend.text = element_text(size = 14)) +
#palette color for bar fills; see Barbiero for reference
  scale_fill_manual(values = c(Barb_Palette)) +
...
gg_Rel_Epi_Zoop_Bio
#ggsave(filename = 'Areal_Epi_Bio_Zoop_Prop.TIFF', plot = gg_Rel_Epi_Zoop_Bio, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/EPI/Areal")


gg_Rel_Epi_Zoop_Den <- ggplot(Areal_Epi_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Abun, fill = SUMGROUP)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
...
gg_Rel_Epi_Zoop_Den 
#ggsave(filename = 'Areal_Epi_Den_Zoop_Prop.TIFF', plot = gg_Rel_Epi_Zoop_Den, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/EPI/Areal")


Epi_All_Zoop_Figure <- ggarrange(gg_Epi_DEN_All, gg_Epi_BIO_All, gg_Rel_Epi_Zoop_Den, gg_Rel_Epi_Zoop_Bio, 
         labels = c("A", "B", "C", "D"),
         font.label = list(size = 14, face = "bold"),
         label.x = c(0, 0, 0, 0), 
         label.y = c(1, 1, 1.05, 1.05),
...
#ggsave(filename = 'Areal_Epi_Zoop_Grid.TIFF', plot = Epi_All_Zoop_Figure, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Figures/EPI/Areal")
```
...
Epi_All_Stats$Season <- factor(Epi_All_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall"))

#Invasive_Zoops_JKC_All_Stats$Year <- factor(Invasive_Zoops_JKC_All_Stats$Year, levels = c(2012,2013,2014,2015,2016,2017,2018,2019))

gg_DEN_All_Epi_Total <- ggplot(Epi_All_Stats, aes(x = Season, y = Zoop_All_Abun_MEAN))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
...
gg_DEN_All_Epi_Total

#ggsave(filename = 'Avg_Den_All_Epi_Total.TIFF', plot = gg_DEN_All_Epi_Total, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/Epi")

#Total Summed avg abun values and total Summed error bars for zooplankton
gg_BIO_All_Epi_Total <- ggplot(Epi_All_Stats, aes(x = Season, y = Zoop_All_Bio_MEAN))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
...
gg_BIO_All_Epi_Total
#ggsave(filename = 'Avg_Bio_All_Epi_Total.TIFF', plot = gg_BIO_All_Epi_Total, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/Epi")

Epi_All_Stats_Zoop_Figure <- ggarrange(gg_DEN_All_Epi_Total, gg_BIO_All_Epi_Total,
        labels = c("A", "B"),
         font.label = list(size = 14, face = "bold"),
         label.x = c(0, 0), 
         label.y = c(1, 1),
...
Epi_All_Stats_Zoop_Figure

#ggsave(filename = 'Epi_All_Zoop_Stats_Figure.TIFF', plot = Epi_All_Stats_Zoop_Figure, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/Figures/Epi")
```
...
WWC_All_Stats$Season <- factor(WWC_All_Stats$Season, levels = c("Early Spring", "Late Spring", "Early Summer", "Summer", "Late Summer", "Early Fall", "Late Fall"))

gg_DEN_All_WWC_Total <- ggplot(WWC_All_Stats, aes(x = Season, y = Zoop_All_Abun_MEAN))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
  geom_errorbar(aes(ymax = Zoop_All_Abun_MEAN + Zoop_All_Den_SE, ymin = Zoop_All_Abun_MEAN - Zoop_All_Den_SE, position = "identity", width = .25)) +
...
gg_DEN_All_WWC_Total
#ggsave(filename = 'Avg_Den_All_WWC_Total.TIFF', plot = gg_DEN_All_WWC_Total, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")

#Total Summed avg abun values and total Summed error bars for zooplankton
gg_BIO_All_WWC_Total <- ggplot(WWC_All_Stats, aes(x = Season, y = Zoop_All_Bio_MEAN))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
  geom_errorbar(aes(ymax = Zoop_All_Bio_MEAN + Zoop_All_Bio_SE, ymin = Zoop_All_Bio_MEAN - Zoop_All_Bio_SE, position = "identity", width = .25)) +
...
gg_BIO_All_WWC_Total
#ggsave(filename = 'Avg_Bio_All_WWC_Total.TIFF', plot = gg_BIO_All_WWC_Total, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")

WWC_All_Stats_Zoop_Figure <- ggarrange(gg_DEN_All_WWC_Total, gg_BIO_All_WWC_Total,
        labels = c("A", "B"),
         font.label = list(size = 14, face = "bold"),
         label.x = c(0, 0), 
         label.y = c(1, 1),
...
WWC_All_Stats_Zoop_Figure

#ggsave(filename = 'WWC_All_Zoop_Stats_Figure.TIFF', plot = WWC_All_Stats_Zoop_Figure, width = 10, height = 8, path = "F:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
```
...
```{r, defining data frame column variables, echo = FALSE, warning = FALSE}
Zoop_Bio_Den_New <- read_csv("C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Updated Raw Data/Zoop_Bio_Den_New.csv")

Areal_Bio_Den_Zoop <- read_csv("C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Updated Raw Data/Areal_Bio_Den_Zoop.csv")
...
Whole_SPECCODE <- Areal_Bio_Den_Zoop$SPECCODE
Whole_SUMGROUP <- Areal_Bio_Den_Zoop$SUMGROUP
Whole_Density <- Areal_Bio_Den_Zoop$Density_L
Whole_Biomass <- Areal_Bio_Den_Zoop$Biomass_L
Whole_Station <- Areal_Bio_Den_Zoop$Station
Whole_DN <- Areal_Bio_Den_Zoop$DN
Whole_EventID <- Areal_Bio_Den_Zoop$EventCode
Whole_Basin <- Areal_Bio_Den_Zoop$Basin
...
Epi_Macro_Zeros <- Epi_Macro_Filter %>% 
    filter(SPECCODE != "LETKIND" & SPECCODE != "POYPEDI" & SPECCODE != "VELIGER" & SPECCODE != "ROTIFER") %>% 
    group_by(SUMGROUP, Season, Station, EventID, DN, Lat, Long) %>% 
      summarise(Zoop_Bio_Sum = sum(Bio.Liter_New), Zoop_Den_Sum = sum(Den.Liter_New)) %>% 
      ungroup %>% 
      complete(nesting(Season, Station, EventID, DN, Lat, Long), SUMGROUP, fill = list(Zoop_Bio_Sum = 0, Zoop_Den_Sum = 0))
...
#write.csv(Complete_Epi_Zoop_Bio_Den_New, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Complete_Epi_Zoop_Bio_Den_New.csv")
```
...
#write.csv(Complete_WWC_Zoop_Bio_Den_New, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/Complete_WWC_Zoop_Bio_Den_New.csv")
```
...
##############Code edited for SUMGROUP instead of Zoop_GENERA########### Also excluded micro sumgroup data ### Excluding Pred Cla 2/8/2023
Epi_Sum_Stats <- Complete_Epi_Zoop_Bio_Den_New %>% 
  filter(SUMGROUP != "OTHER" & SUMGROUP != "MYSID" & SUMGROUP != "VELIGER" & SUMGROUP != "ROTIFER" & SUMGROUP != "pNAUPLII" & SUMGROUP != "BYTH" & SUMGROUP != "CERC") %>% 
  group_by(SUMGROUP, Season) %>%
  summarise(All_Bio_SUM = sum(Zoop_Bio_Sum), 
            All_Den_SUM = sum(Zoop_Den_Sum),
            All_Bio_MEAN = mean(Zoop_Bio_Sum), 
            All_Den_MEAN = mean(Zoop_Den_Sum), 
...
#write.csv(Epi_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Epi_Sum_Stats.csv")

#write.csv(Whole_Epi_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/PRED_CLA_Epi_Sum_Stats.csv")
...
#write.csv(Epi_All_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/Epi/Epi_SumGroup_Macro_All_Stats.csv")
```
...
#write.csv(WWC_Sum_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/WWC_Sum_Stats")
...
#write.csv(WWC_All_Stats, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/WWC_Sum_Stats")
```
...
#plotting average biomass for all taxa from epi per month 
gg_BIO_All <- ggplot(Whole_WWC_Sum_Stats, aes(x = Season, y = All_Bio_MEAN, fill = SPECCODE)) + 
  geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 26), plot.title = element_blank()) +
...
#ggsave(filename = 'Avg_Pred_Cla_WWC_Bio.TIFF', plot = gg_BIO_All, width = 10, height = 8, path = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Figures/WWC")

#plotting average density for all taxa from epi per month
gg_DEN_All <- ggplot(Whole_WWC_Sum_Stats, aes(x = Season, y = All_Den_MEAN, fill = SPECCODE))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 26), plot.title = element_blank()) +
...
#ggsave(filename = 'Avg_Pred_Cla_WWC_Den.TIFF', plot = gg_DEN_All, width = 10, height = 8, path ="C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Figures/WWC")
```
...
WWC_Zoop_Prop <-
  Whole_WWC_Sum_Stats %>%
    select(SPECCODE, Season, All_Bio_MEAN, All_Den_MEAN) %>%
    group_by(Season)%>%
    mutate(Zoop_Prop_Bio = (All_Bio_MEAN / sum(All_Bio_MEAN) * 100), Zoop_Prop_Abun = (All_Den_MEAN / sum(All_Den_MEAN) * 100))

write.csv(Pred_Cla_WWC_Zoop_Prop, file = "C:/Users/Gl Lab User/Desktop/CSMI 2018 Zooplankton/CSMI-2018-Zooplankton/CSMI-2018-Zooplankton/Processed Data/WWC/WWC_Sum_Stats")
```


```{r zoop genera relative abundance and biomass stack graph, echo = FALSE, warning = FALSE, fig.width = 7, fig.height = 5}
gg_Rel_Zoop_Bio <- ggplot(WWC_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Bio, fill = SUMGROUP)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 26), plot.title = element_blank()) +
...
#ggsave(filename = 'Avg_Zoop_WWC_Bio_PROP.TIFF', plot = gg_Rel_Zoop_Bio, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")

gg_Rel_Zoop_Den <- ggplot(WWC_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Abun, fill = SUMGROUP)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 22), axis.title.y = element_text(size = 26), plot.title = element_blank()) +
...
#ggsave(filename = 'Avg_Zoop_WWC_Den_PROP.TIFF', plot = gg_Rel_Zoop_Den, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
...
#ggsave(filename = 'WWC_All_Zoop_Figure.TIFF', plot = WWC_All_Zoop_Figure, width = 10, height = 8, path = "F:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
```
...
#plotting average biomass for all taxa from epi per month 
gg_Epi_BIO_All <- ggplot(Whole_Epi_Sum_Stats, aes(x = Season, y = All_Bio_MEAN, fill = SPECCODE)) + 
  geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
#legend details 
  theme(legend.title = element_blank(), legend.position = "top", legend.box.background = element_rect(), legend.box.margin = margin(3,3,3,3), legend.text = element_text(size = 20)) +
#palette color for bar fills; see Barbiero for reference
  scale_fill_manual(values = c(Barb_Palette)) +
  labs(x = "Month", y = "Biomass (µg/L)") +
  ylim(0,4)
...
#ggsave(filename = 'Pred_Cla_Epi_Bio.TIFF', plot = gg_Epi_BIO_All, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")

#plotting average density for all taxa from epi per month
gg_Epi_DEN_All <- ggplot(data = Whole_Epi_Sum_Stats, aes(x = Season, y = All_Den_MEAN, fill = SPECCODE))  + 
   geom_bar(color = "black", stat = "identity") + 
  #panel border, axis text and title sizes and orientations
  theme_cowplot() +
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_blank(), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
...
#ggsave(filename = 'Pred_Cla_Epi_Den.TIFF', plot = gg_Epi_DEN_All, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
```
...
gg_Rel_Epi_Zoop_Bio <- ggplot(Epi_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Bio, fill = SUMGROUP)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
...
gg_Rel_Epi_Zoop_Bio
#ggsave(filename = 'Avg_Rel_Zoop_Epi_Bio.TIFF', plot = gg_Rel_Epi_Zoop_Bio, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")

gg_Rel_Epi_Zoop_Den <- ggplot(Epi_Zoop_Prop, aes(x = Season, y = Zoop_Prop_Abun, fill = SUMGROUP)) +
    geom_bar(color = "black", stat = "identity") + 
  theme_cowplot() +
  #panel border, axis text and title sizes and orientations
 theme(panel.border = element_rect(linetype = "solid", fill = NA), axis.text.x = element_text(size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), axis.title.x = element_blank(), axis.text.y = element_text(size = 24), axis.title.y = element_text(size = 28), plot.title = element_blank()) +
...
gg_Rel_Epi_Zoop_Den 
#ggsave(filename = 'Avg_Rel_Zoop_Epi_Den.TIFF', plot = gg_Rel_Epi_Zoop_Den, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
...
#ggsave(filename = 'Epi_All_Zoop_Figure.TIFF', plot = Epi_All_Zoop_Elements_Figure, width = 10, height = 8, path = "F:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
```
...
gg_DEN_All_Epi_Total
#ggsave(filename = 'Avg_Den_All_Epi_Total.TIFF', plot = gg_DEN_All_Epi_Total, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
...
gg_BIO_All_Epi_Total
#ggsave(filename = 'Avg_Bio_All_Epi_Total.TIFF', plot = gg_BIO_All_Epi_Total, width = 10, height = 8, path = "E:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
...
#ggsave(filename = 'Epi_All_Zoop_Stats_Figure.TIFF', plot = Epi_All_Stats_Zoop_Figure, width = 10, height = 8, path = "F:/CCM Work Files - Desktop Version/CSMI Analysis/Ontario 2018/CCM Zoop Assessment/Figures")
```
...
>>>>>>> 8e2bb8e0c4e6486ef37f3ef7be71b6c53582619e
